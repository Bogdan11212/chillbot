...
class Members(commands.Cog):
  ...
  async def on_member_join(self, member):
    channel = self.bot.get_channel(1100999888763887666)
    role = member.guild.get_role(1128631519787888715)
    ...
    embed = disnake.Embed(
        color=0xffffff,
        description=f"Добро пожаловать {member.mention} на сервер angelrise 😊 а я тут бот-помощник😋♥️"
    )
